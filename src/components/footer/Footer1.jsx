...
const Footer1 = () => {
  ...
  return (
    <div className="container mx-auto p-5 sm:p-10">
      <div className="grid md:grid-cols-[auto_auto_auto_auto] md:place-items-center gap-10 border-b border-[#f0f0f0] dark:border-[#252528] pb-16">
        {cols.map((col, _) => (
          <div key={_} className="h-full">
            <div className="font-bold mb-5 text-lg">{col.title}</div>
            <ul className="space-y-2">
              {col.links.map((colLink, i) => (
                <li
                  key={i}
                  className="text-[#4d4d4d] dark:text-[#ababab] hover:text-[#141414] dark:hover:text-[#eeeeee] text-sm transition-all  hover:translate-x-1 "
                >
